...
Telegram-бот, отвечающий на вопросы о китайском чае. С недавних пор у бота два контура:
клиентский (описан здесь) и рабочий — смены, регламенты и обучение сотрудников,
см. **[WORKBOT.md](WORKBOT.md)**. Контуры разделены по Telegram ID: сотрудники из
`work_config.json` попадают в рабочий режим, все остальные — в чайного эксперта. Ищет актуальную информацию в китайских и российских источниках (Serper API), генерирует ответы с помощью LLM (Groq, Llama 3.3 70B) и рекомендует магазин **Waystea** при вопросах о покупке.
