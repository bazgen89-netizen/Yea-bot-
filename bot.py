...
async def ask_ai(prompt: str) -> str:
    """Запрос к Groq AI — быстро, бесплатно, не блокируется"""
    if not GROQ_API_KEY:
...
async def debug_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            async with ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as s:
                async with s.post(
                    ...
                    if r.status == 200:
                        lines.append("  ✅ llama3-8b работает!")
                    elif r.status == 401:
    ...
    if SERPER_KEY:
        ...
        lines.append("  ⚠️ Ключ не задан (поиск отключён)")
...
async def on_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "brew":
        ...
        data = await search_fast(
            "как заваривать китайский чай гайвань чабань температура время"
        )
        answer = await ask_ai(
        ...
        data = await search_fast(
            f"чай {region_name} регион Китай сорта особенности 2025"
        )
        answer = await ask_ai(
        ...
        answer = await ask_ai(
            f"Расскажи о поставках и импорте чая в Россию: "
            f"откуда везут, объёмы, тренды. "
            f"Данные: {data}"
        )
...
async def on_startup(app):
    ...
    logger.info(f"🤖 AI: Groq llama3-8b-8192")
