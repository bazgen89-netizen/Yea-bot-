...
- [События, которые приходят сами](#события-которые-приходят-сами)
- [Команды](#команды)
- [Автопилот](#автопилот)
...
1. Фоновая задача каждые `SOCIAL_POLL_INTERVAL` секунд (по умолчанию 180)
   опрашивает все настроенные площадки. События Meta приходят сами —
   см. [ниже](#события-которые-приходят-сами).
...
## События, которые приходят сами

Опрос работает не везде. WhatsApp вообще не отдаёт входящие по запросу —
Meta присылает их на webhook. Для Instagram и Facebook webhook убирает
задержку: сообщение появляется в чате сразу, а не через три минуты.

Точки приёма — на том же адресе, где висит webhook Telegram:
**`/social/meta`** для площадок Meta и **`/social/vk`** для ВКонтакте.

| Переменная | Зачем |
|---|---|
| `META_VERIFY_TOKEN` | Придумайте строку — Meta подтвердит ею подписку. Без неё точка приёма отключена |
| `META_APP_SECRET` | App Secret приложения Meta: проверка подписи `X-Hub-Signature-256`. Без него подпись не проверяется — задайте обязательно |

Настройка в Meta for Developers → ваше приложение → Webhooks:

1. Callback URL: `https://ваш-адрес/social/meta`
2. Verify token: значение `META_VERIFY_TOKEN`
3. Подписки на поля: `messages` (WhatsApp), `comments` и `messages`
   (Instagram), `feed` и `messages` (страница Facebook)

### ВКонтакте

По запросу у ВКонтакте читаются только диалоги — **комментарии под
постами приходят событием Callback API**. Без него бот их не увидит.

| Переменная | Зачем |
|---|---|
| `VK_CONFIRMATION` | Строка подтверждения из настроек сообщества. Без неё точка приёма отключена |
| `VK_CALLBACK_SECRET` | Секретный ключ сообщества: проверка, что событие от ВКонтакте |

Настройка в сообществе → Управление → Работа с API → Callback API:

1. Адрес: `https://ваш-адрес/social/vk`
2. Скопировать строку подтверждения в `VK_CONFIRMATION`, задать секретный ключ
3. Типы событий: «Комментарии на стене», «Входящие сообщения»,
   при необходимости комментарии к фото и видео

Ответ на комментарий уходит комментарием в ту же ветку
(`wall.createComment`), на сообщение — в диалог. Смешивать нельзя: автор
комментария нам не писал, личное сообщение он может и не получить.

### Общее для обеих точек приёма

Бот отвечает площадке сразу, а разбор и отправку карточки делает фоном:
иначе доставка считается неудачной и событие присылают снова.
Повторы безопасны — `SeenStore` отсеивает уже показанное.

## Команды

| Команда | Что делает |
|---|---|
| `/social` | Панель: площадки, сгруппированные по магазинам, статус автопилота, ошибки |
...
| фото с подписью | То же самое, но с картинкой: пришлите боту фото — подпись станет текстом поста |
...
Instagram без картинки не публикует, и картинку он берёт **только по
публичной ссылке** — файл ему отдать нельзя. Ссылка Telegram не подходит:
она временная и содержит токен бота. Поэтому картинка сначала уезжает в
медиатеку сайта, а площадкам уходит постоянный адрес оттуда.

| Переменная | Зачем |
|---|---|
| `WP_URL` | Адрес сайта, по умолчанию `https://waystea.ru` |
| `WP_USER` | Логин WordPress |
| `WP_APP_PASSWORD` | Пароль приложения WordPress (не пароль от учётки) |

Как выглядит работа: присылаете боту фото с подписью → он выкладывает
картинку и показывает, куда собирается публиковать → по кнопке пост
уходит во все сети сразу. Фото без подписи тоже подойдёт — бот попросит
текст следующим сообщением.

Требования Instagram к картинке: **JPEG**, до 8 МБ, соотношение от 4:5
до 1.91:1. Бот проверяет формат и размер до загрузки и предупреждает,
если пост в Instagram не пройдёт, — остальные сети при этом опубликуются.

Куда попадает картинка в каждой сети: Instagram — пост с подписью,
Facebook — фото-пост, Telegram-канал — фото с подписью (до 1024 знаков),
ВКонтакте и Одноклассники — текстом, без картинки.
...
Входящие WhatsApp приходят на webhook Meta, а не по запросу: включите
точку приёма `/social/meta` (см. [выше](#события-которые-приходят-сами)),
иначе бот не увидит ни одного сообщения от клиентов.
...
|---|:---:|:---:|:---:|
| ВКонтакте | ✅ диалоги (опрос) + комментарии (webhook) | ✅ | ✅ |
| Telegram-канал | — | — | ✅ |
| Одноклассники | — | — | ✅ |
| Facebook (страница) | ✅ (опрос + webhook) | ✅ | ✅ |
| Instagram | ✅ (опрос + webhook) | ✅ | ✅ (только с картинкой) |
