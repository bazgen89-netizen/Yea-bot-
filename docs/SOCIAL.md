...
Одна точка приёма обслуживает все три площадки: **`/social/meta`**
на том же адресе, где висит webhook Telegram.
...
Бот отвечает Meta сразу, а разбор и отправку карточки делает фоном:
иначе Meta считает доставку неудачной и присылает событие снова.
Повторные доставки безопасны — `SeenStore` отсеивает уже показанное.
