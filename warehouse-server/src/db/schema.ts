...
export const MIGRATIONS: string[] = [
  ...
  // 4 — продажа при нехватке товара
  `
  -- Разрешать ли уводить остаток в минус на кассе.
  -- По умолчанию да: в рознице товар нередко отдают раньше, чем оформляют
  -- приход, и отказ провести чек означал бы потерянную выручку в учёте.
  ALTER TABLE orgs ADD COLUMN allow_negative_stock BOOLEAN NOT NULL DEFAULT true;
  `,
  ...
  /**
   * Метки CRM: «опт», «бар», «подарочники».
   *
   * Строкой в самой карточке, а не отдельной таблицей связей: их у клиента
   * две-три, карточка и так ездит между магазинами, и так метка уезжает
   * вместе с ней — без второй таблицы и второго обмена.
   */
  `
