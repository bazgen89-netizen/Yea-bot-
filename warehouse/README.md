...
Сборка кладёт всё приложение в один файл `dist/index.html` — код, SQLite
в WebAssembly и шрифты иконок внутри, ни одного стороннего запроса. Такой файл
можно просто выложить на любой статический хостинг и открыть по ссылке.

В браузере база своя, на sql.js, а не на `expo-sqlite`: веб-версия последней
работает через `SharedArrayBuffer`, а его браузер выдаёт только странице
с заголовками `Cross-Origin-Opener-Policy` и `Cross-Origin-Embedder-Policy` —
обычный хостинг их не ставит, и приложение бы не открылось. Данные лежат
в IndexedDB и переживают перезапуск браузера. На телефоне всё как раньше:
нативный SQLite.
...
│   │   ├── expoDriver.ts     база на телефоне — нативный SQLite
│   │   ├── expoDriver.web.ts тот же вход для браузера, без expo-sqlite
│   │   └── webDriver.ts      база в браузере — sql.js, файл в IndexedDB
│   ├── state/                провайдеры базы, корзины и сканера
│   └── ui/                   тема и переиспользуемые компоненты
├── scripts/build-web.mjs     сборка веб-версии в один файл
