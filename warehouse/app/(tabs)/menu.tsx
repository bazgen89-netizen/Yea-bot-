...
function useSections(): { title: string; rows: MenuRow[] }[] {
  ...
  const sections: { title: string; rows: MenuRow[] }[] = [
    // ИИ-помощник первым: на телефоне это чат на весь экран. Прежний
    // «Спросить у склада» был снят по просьбе Вазгена до переделки — вот
    // она: вместо анкеты разговор, и модели бесплатные на выбор.
    {
      title: 'Помощник',
      rows: [{ label: 'ИИ-помощник', icon: 'assistant', href: '/assistant' }],
    },
    {
      title: 'Клиенты',
      rows: [
