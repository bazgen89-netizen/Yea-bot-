 ...
 * комплект».
 *
 * Значит правило такое: комплект — тот, кого упоминают в чужом составе.
 * Проверено на его данных: упомянутых 92, из них 88 начинаются с «В зале»,
 * остальные — «… доп» и «Бутылка холодного чая»; в общем списке `/product`
 * их нет ни одной, они дозагружаются поштучно из истории.
 */
...
const card = (item, archived) => {
  ...
  return {
    ...
    k: комплекты.has(String(item.id)) ? 'set' : undefined,
