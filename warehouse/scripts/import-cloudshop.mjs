...
/** Кого CloudShop действительно снял с продажи — по идентификаторам. */
const снятыеИд = new Set(
  (Array.isArray(rawDeleted) ? rawDeleted : []).map((item) => String(item.id)),
);
 ...
 * Кто у него комплект.
 *
 * У CloudShop вид позиции отдельным полем не отдаётся вовсе — в карточке
 * есть только `components`. Читается оно наоборот тому, что подсказывает
 * название: у обычного товара «Дикорастущий Сяо Чжун» в `components` лежит
 * «В зале Дикорастущий Сяо Чжун», а сама «В зале …» приходит с пустым
 * составом. При этом в его кабинете «К» стоит именно у «В зале …», а у
 * «Дикорастущего» — «Т». Вазген это и сказал: «например, в зале это
 * комплект».
 *
 * Правил два, и второе — от Вазгена.
 *
 * Первое: комплект — тот, кого упоминают в чужом составе. Упомянутых 92, из
 * них 88 начинаются с «В зале», остальные — «… доп» и «Бутылка холодного
 * чая»; в общем списке `/product` их нет ни одной, они дозагружаются
 * поштучно из истории.
 *
 * Второе: **всё, что в зале, — комплект**. Так сказал Вазген: «все позиции,
 * которые в зале… везде напротив тех, кто протест, а не буква К».
 *
 * «В зале» — это и название, и категория, и проверять надо оба. Сперва я
 * смотрел только на начало названия и промахнулся на семнадцати живых
 * карточках: у «Матча тоник», «Камбуча 0.25л», «Варенье в ассортименте» и
 * четырнадцати сертификатов категория «в зале», а в названии этих слов нет
 * вовсе. Он их и увидел товарами.
 *
 * По составам под это правило не попадают ещё девять карточек — «В зале
 * Пур Га», «В зале Шэн Пуэр 3-5 лет» и соседние: в самом CloudShop они
 * записаны товарами, потому что ни в чей состав не входят. Но чайную
 * держит он, и «в зале» у него значит комплект. Расхождение с его же
 * кабинетом тут сознательное, и ему сказано.
 */
const вЗале = (...строки) => строки.some((одна) => /в\s+зале/i.test(одна ?? ''));
const комплекты = new Set();
for (const item of rawProducts) {
  for (const часть of item.components ?? []) if (часть?.id) комплекты.add(String(часть.id));
}
console.log(`  из них комплектов (по составам): ${комплекты.size}`);
...
const card = (item, archived) => {
  ...
  return {
    ...
    img: item.img?.[0] ?? null,
    // Вид позиции: «set» — комплект, иначе обычный товар. Пишем только
    // комплекты, чтобы не тащить слово «product» шестьсот раз.
    k: комплекты.has(String(item.id)) || вЗале(options.name, item.tags?.[0]) ? 'set' : undefined,
  ...
  if (lost.size) {
    ...
    for (const id of lost.keys()) {
      try {
        ...
        if (item && item.id) {
          /*
           * Архивной такая карточка становится не всегда.
           *
           * Раньше все дозагруженные заводились снятыми с продажи — на том
           * основании, что их нет ни в живом списке, ни в снятых. Но список
           * `/product` у него неполный: «В зале Дикорастущий Сяо Чжун» в нём
           * нет, среди снятых его тоже нет, а в кабинете он живой и стоит с
           * пометкой «К». Из-за этого у нас в справочнике было 593 позиции
           * против его 684 — ровно на число комплектов.
           *
           * Поэтому живыми заводятся комплекты, которых нет среди снятых:
           * их CloudShop использует прямо сейчас, раз они стоят в чужих
           * составах. Всё остальное, что встретилось только в истории,
           * по-прежнему архив — этих карточек в его справочнике нет.
           */
          const живой = комплекты.has(String(item.id)) && !снятыеИд.has(String(item.id));
          products.push(card(item, !живой));
          got++;
        }
      } catch {
