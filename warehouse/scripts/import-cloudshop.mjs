 ...
 * поштучно из истории.
 *
 * Второе: **всё, что называется «В зале …», — комплект**. Так сказал
 * Вазген: «там, где в зале, напротив каждой позиции надо поставить другую
 * букву, не Т». По составам под это правило не попадают девять его живых
 * карточек — «В зале Пур Га», «В зале Шэн Пуэр 3-5 лет» и соседние: в самом
 * CloudShop они записаны товарами, потому что ни в чей состав не входят.
 * Но чайную держит он, и «в зале» у него значит комплект — значит комплект.
 * Расхождение с его же кабинетом тут сознательное, и ему сказано.
 */
const вЗале = (имя) => /^\s*в\s+зале/i.test(имя ?? '');
...
const card = (item, archived) => {
  ...
  return {
    ...
    k: комплекты.has(String(item.id)) || вЗале(options.name) ? 'set' : undefined,
