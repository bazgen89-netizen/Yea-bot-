...
describe('реестр отчётов', () => {
  ...
  it('считает продажи и остатки', () => {
    ...
    // себестоимость продаж, продажи, продано, рентабельность, маржинальность.
    expect(byProduct[0]).toEqual([
      ...
      '100%',
      // Маржинальность — та же прибыль, но к выручке: 2000 из 4000 это 50 %.
      // Две дроби с одним числителем, и путать их нельзя: в его списке
      // «Параметр» они стоят рядом.
      '50%',
    ]);
