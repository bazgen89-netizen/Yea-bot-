...
export interface AskResult {
  columns: string[];
  /**
   * Найденные строки — все, а не показанные двести.
   *
   * Держать их здесь нужно, чтобы упорядочить и отобрать можно было на
   * месте, мгновенно и бесплатно: переспрашивать модель ради другого порядка
   * значило бы платить за то, что и так посчитано.
   */
  rows: unknown[][];
  /** Сколько строк нашлось всего — может быть больше, чем сюда влезло. */
  total: number;
}

/**
 * Сколько строк держать в памяти.
 *
 * Не «сколько показать» — показывает экран, и он показывает двести. Держим
 * больше, чтобы упорядочить и отобрать по всему найденному, а не по первым
 * двумстам: иначе «по убыванию» переставляло бы двести строк между собой, а
 * самое большое оставалось бы за краем.
 */
const ДЕРЖИМ = 5_000;
...
export function runSql(db: SqlDriver, raw: string): AskResult {
  ...
  return {
    columns,
    rows: rows.slice(0, ДЕРЖИМ).map((row) => columns.map((column) => row[column])),
    total: rows.length,
  };
}
