...
export const MIGRATIONS: string[] = [
  ...
  // 22. Свой номер документа.
  //
  // В журнале чек подписан своим внутренним номером — «Продажа #1». Пока
  // база заводится с нуля, это и есть его номер. Но у перенесённой истории
  // номер уже был: в CloudShop тот же чек называется «Продажа #45658», и
  // именно этот номер он ищет, сверяет и называет вслух. Внутренний номер
  // рядом с ним — чужой.
  `
  ALTER TABLE sales ADD COLUMN number INTEGER;
  CREATE INDEX IF NOT EXISTS idx_sales_number ON sales(number);
  -- В движении денег тот же чек называется иначе — «Приход #46047». Это
  -- вторая, своя нумерация: приходов больше, чем чеков, потому что деньги
  ...
  // 25 — скидка в строке чека.
  //
  // Скидка хранилась одна на весь чек, а в документе она стоит **в строке**:
  // у него в «Продаже #45904» первая позиция продана за 170.10 со скидкой
  // 170.10 и итогом 0.00, а остальные — без скидки. Одной суммой на чек это
  // не выразить: непонятно, что именно отдали даром.
  //
  // В перенесённой истории скидка по строкам уже есть — приезжает из
  // CloudShop вместе с чеком, и до сих пор просто отбрасывалась.
  `
  ALTER TABLE sale_items ADD COLUMN discount INTEGER NOT NULL DEFAULT 0;
  `,

  // 26 — возврат продажи среди перенесённых документов.
  //
  // У возвратов в CloudShop свой раздел и своя нумерация — «Возврат продажи
  // #11», — и в журнале они стоят рядом с чеками. Свои возвраты мы узнаём по
  // движению склада с причиной `return`, но у перенесённых движений нет
  // вовсе: история склад не трогает. Поэтому признак хранится явно.
  `
  ALTER TABLE sales ADD COLUMN is_return INTEGER NOT NULL DEFAULT 0;
  `,

  // 27 — когда карточку товара правили в последний раз.
  //
  // В его окне «Фильтр» есть поле «Изменения товара — изменялся в течение N
  // дней», а отвечать на него было нечем: дата правки нигде не хранилась.
  // У карточек, которых с тех пор не касались, поле остаётся пустым — это
  // честнее, чем проставить им день переноса и объявить все изменёнными.
  `
  ALTER TABLE products ADD COLUMN updated_at TEXT;
  `,

  // 28 — касса и смена у перенесённых чеков.
  //
  // В его документе стоят «Касса: №1» и «Смена: #3430», а у нас в этих
  // строках был прочерк. Ключ отдаёт только внутренние номера
  // (`references.register_id`, `shift_id`) — ни названия кассы, ни номера
  // смены. Название кассы перенос достаёт из соседней записи магазина, а
  // смены нумерует сам, по порядку первых чеков.
  //
  // Своя касса и своя смена хранятся ссылками на `registers` и `shifts`;
  // здесь — то, что приехало из CloudShop и своих записей не имеет.
  `
  ALTER TABLE sales ADD COLUMN register_name TEXT;
  ALTER TABLE sales ADD COLUMN shift_no      INTEGER;
  `,

  // 29 — имя покупателя отдельно от комментария
  //
  // Раньше имя покупателя, чью карточку не удалось сопоставить, писалось в
  // `note` — то есть в комментарий к чеку. Из-за этого в журнале у таких
  // чеков стоял значок заметки, а в документе в графе «Комментарий»
  // оказывалось имя человека. Комментарий к продаже у CloudShop свой
  // (`comment` в описи ключа), и мешать их нельзя.
  `
  ALTER TABLE sales ADD COLUMN customer_name TEXT;
  `,

  // 30 — автор складского документа учётной записью
  //
  // У своих документов автор — сотрудник (`staff_id`), и этого хватало. Но
  // перенесённые из CloudShop корректировки заведены не нашим сотрудником, а
  // учётной записью кабинета — «waystea», «Чайный бар». В журнале колонка
  // «Автор» у них оставалась пустой, хотя в кабинете имя стоит. У чеков для
  // этого своя колонка `author` — теперь такая же есть и у документов.
  `
  ...
  /**
   * Чужое имя для нашей же записи.
   *
   * Один и тот же «Чайный бар» может быть заведён на двух устройствах порознь
   * — до того, как они впервые списались. Тогда у одной и той же точки два
   * разных `uid`, и оба настоящие: под своим её знает касса, под чужим —
   * телефон кладовщика. Второй «Чайный бар» заводить нельзя (имя занято, да
   * и в жизни он один), а забыть чужое имя тоже нельзя: чеки с того
   * устройства ссылаются на точку именно им, и без него они повиснут.
   *
   * Поэтому чужое имя запоминается здесь и дальше ведёт к нашей строке.
   * Своё имя записи не меняется никогда — на него уже ссылается сервер.
   *
   * Ровно на это я и напоролся на живой проверке: вход со второго устройства
   * падал с «UNIQUE constraint failed: locations.name».
   */
  `
