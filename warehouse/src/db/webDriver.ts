...
export async function openWebDatabase(): Promise<SqlDriver> {
  ...
  if ((globalThis as { __DEMO__?: boolean }).__DEMO__) {
    ...
    //
    // Каждая часть наполнения помечается в базе и второй раз не заводится:
    // без этого повторный запуск задваивал бы каталог. Но браузер держит
    // базу от прежнего файла, и когда приходит следующий — со свежей
    // выгрузкой из CloudShop, — отметки уже стоят, и новые данные не
    // грузятся. Со стороны это выглядит так, будто перенос не сработал:
    // товары старые, истории покупок нет.
    //
    // Поэтому сверяем подпись выгрузки. Другая — заводим заново. Терять тут
    // нечего: вся база этой сборки и есть привезённая выгрузка.
    // Отсутствие подписи — тоже несовпадение, и это не мелочь: базы,
