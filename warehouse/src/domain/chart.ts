...
import { formatMoneyWeb } from './money';
...
/**
 * Что стоит под курсором по высоте — для плашки-линейки слева на оси.
 *
 * Вазген прислал запись экрана: у него за курсором тянется горизонтальный
 * пунктир, а слева на оси висит плашка с числом — «32,767.76». Это не выручка
 * дня: на записи видно, как число меняется, пока мышь идёт вверх-вниз над
 * одним и тем же днём. Линейка показывает, сколько было бы на этой высоте.
 *
 * Считается от вершины оси: верх графика — `peak`, низ — ноль. За края не
 * выходит: мышь попадает и на пиксель выше верхней линии, и без ограничения
 * плашка показала бы больше вершины.
 */
export function подРукой(peak: number, y: number, height: number): string {
  if (height <= 0) return formatMoneyWeb(0);

  const доля = Math.min(1, Math.max(0, 1 - y / height));
  return formatMoneyWeb(Math.round(peak * доля));
}
