...
/**
 * Итог движения денег за показанный период: сколько пришло, сколько ушло и
 * что осталось в сумме.
 *
 * Нужен телефонному «Движению денег»: там таблицы нет, и сверху одной
 * строкой видно главное — «пришло 48 тысяч, ушло 12». Складывать это в
 * самом экране нельзя: арифметика живёт здесь, где её видно тестам.
 */
export function итогДвижения(строки: { income: Kopecks; expense: Kopecks }[]): {
  приход: Kopecks;
  расход: Kopecks;
  разница: Kopecks;
} {
  let приход = 0;
  let расход = 0;
  for (const одна of строки) {
    приход += одна.income;
    расход += одна.expense;
  }
  return { приход, расход, разница: приход - расход };
}
