...
export function ReportPhone({
  ...
  /** Как колонка называется на этом экране. */
  const имяКолонки = (номер: number) =>
    report.columns[номер]?.phoneTitle ?? report.columns[номер]?.title ?? '';
  ...
  /*
   * Порядок строк. Само сравнение — в domain, здесь только правило.
   *
   * У отчёта по датам первая колонка не название, а дата, и сравнивать её
   * буквами нельзя: сентябрь вставал 1, 10, 11, 2, 30, 9. Запрос отдаёт
   * дни по порядку, и для такой колонки порядком служит он сам.
   */
  const упорядочены = useMemo(
    () =>
      порядокСтрок(строки, {
        колонка: сортировка.колонка,
        вверх: сортировка.вверх,
        числовая: Boolean(report.columns[сортировка.колонка]?.numeric),
        поПорядку: Boolean(report.dates) && сортировка.колонка === 0,
      }),
    [строки, сортировка.колонка, сортировка.вверх, report.id, report.dates],
  );
    ...
    <View style={стиль.экран}>
      ...
      <View style={стиль.лента}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={стиль.лентаТело}>
          ...
          <Фишка
            подпись="Параметр"
            значение={имяКолонки(выбор.колонка) || '—'}
        ...
        {видно.map((строка, номер) => {
          ...
          return (
            <View key={номер} style={стиль.строка}>
              ...
              <Text
                style={[стиль.значение, целое.startsWith('-') && стиль.минус]}
                numberOfLines={1}
              >
                {целое}
                <Text style={[стиль.копейки, целое.startsWith('-') && стиль.минусДробь]}>
                  {дробь}
                </Text>
              </Text>
            </View>
      ...
      {итог ? (
        <View style={стиль.итог}>
          ...
          <Text style={[стиль.итогСумма, рубли.startsWith('-') && стиль.минус]}>
            {рубли}
            <Text style={[стиль.итогКопейки, рубли.startsWith('-') && стиль.минусДробь]}>
              {копейки}
            </Text>
          </Text>
        </View>
      ...
      <Окно
        ...
        заголовок="Параметр"
        строки={числовые.map(({ номер }) => ({ ключ: String(номер), имя: имяКолонки(номер) }))}
...
const стиль = StyleSheet.create({
   ...
   * Минус — красным.
   *
   * В «Отчёте по движению» отрицательный остаток — не мелочь оформления,
   * а повод разбираться: товар продан, а прихода на него не заводили.
   * Чёрным он теряется среди сотни строк.
   */
