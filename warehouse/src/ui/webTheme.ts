...
export const pos = {
  ...
  hover: 'var(--pos-hover)',
  /** Вторая подложка: шапка окна, строка списка, полоса прокрутки. */
  raise: 'var(--pos-raise)',
...
/**
 * Тёмное оформление — их собственное, а не подобранное на глаз.
 *
 * Касса CloudShop переключается классом `theme-dark`, и в её таблице стилей
 * прописан весь набор: `#root{background:#000;color:#fff}`, подложка витрины
 * `#1c1c1e`, плитка на ней — чёрная, разделители `rgba(84,84,88,.6)`, серый
 * текст `rgba(235,235,245,.6)`, синий `#0a84ff`, красный `#ff453a`,
 * оранжевый `#ff9f0a`. Это палитра iOS, и подсмотреть её точнее, чем взять
 * из их же файла, нельзя.
 *
 * Заметьте порядок: у них **подложка светлее плитки**, а не наоборот, как в
 * светлой теме. Оттого чёрная карточка на тёмно-сером и читается как
 * карточка — тень в темноте не видна, границу держит разница тонов.
 */
export const POS_DARK: Record<string, string> = {
  ...
  '--pos-bg': '#1C1C1E',
  '--pos-tile': '#000000',
  '--pos-border': 'rgba(84,84,88,0.6)',
  '--pos-text': '#FFFFFF',
  '--pos-muted': 'rgba(235,235,245,0.6)',
  '--pos-hover': 'rgba(255,255,255,0.08)',
  // Вторая подложка: строка под курсором, полоса прокрутки, шапка окна —
  // у них `#2c2c2e`, то есть светлее подложки, а не темнее.
  '--pos-raise': '#2C2C2E',
  // Тень в темноте не работает — вместо неё светлая обводка, как у них
  // (`box-shadow:0 0 5px 1px hsla(0,0%,100%,.1)`).
  '--pos-shadow': 'rgba(255,255,255,0.10)',
  // Выбранная строка у них — синий с прозрачностью, а не сплошной цвет.
  '--pos-select': 'rgba(10,132,255,0.20)',
  '--pos-faint': '#48484A',
};
