...
const ПРЕЖНИЙ_КАБИНЕТ = {
   ...
   * «У» — услуга.
   *
   * Замерено у них: зелёный `rgb(240,253,244)` на заливке, `rgb(22,128,61)`
   * буквой и рамкой, скругление 4, 8,75 точки, поля 2×6. Оранжевый снят с
   * кадра записи. Фиолетовый для услуги — единственное, чего я у него не
   * видел: услуг в его справочнике нет, и показать их нечем.
   *
...
const НОВЫЙ_КАБИНЕТ_ТЁМНЫЙ: typeof ПРЕЖНИЙ_КАБИНЕТ = {
   ...
   * rgb(59,23,2), заливка чипа категории rgb(27,50,83) с буквами
   * rgb(90,156,250).
   */
  cellBorder: '#2E384A',
