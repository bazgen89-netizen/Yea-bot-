 ...
 */
/**
 * Заливка шапки: `linear-gradient(310deg, headerFrom, headerTo)`.
 *
 ...
 * стояли наоборот, и шапка была темнее там, где у него светлее. Поэтому
 * счёт идёт от `headerTo` к `headerFrom`, а не наоборот.
 *
 * Прежде здесь лежали восемь готовых значений, посчитанных руками для
 * прежнего кабинета. Теперь они считаются — и потому шапка перекрашивается
 * вместе с версией: у нового кабинета градиент свой, из другой пары цветов.
 * Прежние восемь эта формула даёт в точности, на это есть тест.
 */
const BANDS = полосыГрадиента(web.headerTo, web.headerFrom, 8);
