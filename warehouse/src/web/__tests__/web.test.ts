...
describe('заголовок раздела', () => {
  ...
  /**
   * Открытый документ подписывается тем, чем он является.
   *
   * Заголовок подставлял «Закупка» всякому документу, у которого вида в
   * адресе не было, — и корректировка, открытая из истории товара,
   * открывалась подписанной закупкой. Заголовок обязан молчать о том, чего
   * не знает.
   */
  it('не выдаёт корректировку за закупку', () => {
    expect(titleFor('/doc/12', 'adjustment')).toBe('Движение товара / Корректировка');
    expect(titleFor('/doc/12', 'inventory')).toBe('Движение товара / Инвентаризация');
    expect(titleFor('/doc/12')).toBe('Движение товара / документ');
  });
