...
export const MENU: MenuEntry[] = [
  ...
  // «Спросить у склада» временно снято по просьбе Вазгена: вернём, когда
  // помощник будет переделан. Экран `/assistant` остался на месте и работает
  // — из меню на него просто не ведёт ни одна строка.
  ...
  // его не откроют.
  /*
   * CRM — внутри «Контрагентов», а не отдельной строкой сверху.
   *
   * Так и по смыслу (работа с клиентами — это про контрагентов), и по месту:
   * у него в кабинете верхних разделов ровно столько, сколько помещается в
   * меню без прокрутки. Пока CRM стояла отдельной строкой, наше меню было на
   * полсотни точек выше окна и превращалось в собственную область прокрутки:
   * наводишь курсор на меню, крутишь колесо — едет меню, а не страница.
   * У него такого нет, меню не прокручивается вовсе.
   */
  {
    label: 'Контрагенты',
    icon: 'parties',
    children: [
      ...
      { label: 'CRM — работа с клиентами', href: '/crm', needs: 'clients' },
    ],
  },
  {
    label: 'Компания',
